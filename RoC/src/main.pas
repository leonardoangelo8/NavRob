...
type
  ...
  TFMain = class(TForm)
    ...
    procedure TimerSimTimer(Sender: TObject);
  ...
  private
    ...
    procedure Omin_Ticks(v,w: double; out w1,w2: double);
    procedure ResetView;
    procedure ShowEdges;
    procedure ShowRadar;

    procedure OminSim(v,w: double; var R: TRobotState);
...
uses Robots, Utils, Field, Tatic, Actions, Tasks, Param, drawmap, MPC;//, Camera, Joy, LCLType, omni3
...
begin
  ...
  if CBSim.Checked=true then begin
    ControlAccelerationNLTMD(v,w);
    //Omin_Ticks(v,w,w1,w2);
    OminSim(v,w,RobotState[myNumber]);
...
procedure TFMain.OminSim(v,w: double; var R: TRobotState);
var
    mte,ce,se: double;
    i:integer;
begin
...
// Ideal Model, based on reference velocities
  ...
  R.v1:= v+(Wheeltowheeldist/2)*w;
  R.v2:= v-(Wheeltowheeldist/2)*w;
...
procedure TFMain.Omin_Ticks(v,w: double; out w1,w2: double);
var
  speed1w,speed2w: double;
begin
  speed1w:= v+(WheelToWheelDist/2)*w;
  speed2w:= v-(WheelToWheelDist/2)*w;

  w1:=speed1w/(WheelDiameter/2);
  w2:=speed2w/(WheelDiameter/2);
end;
...
procedure TFMain.TimerSimTimer(Sender: TObject);
begin
  if CBSim.Checked=true then begin
     SimMainLoop;
  end;
...
begin
  ...
  SScanf(Data,'%s %s',[@mv, @mw]);

  RobotState[myNumber].v:=strtofloatdef(mv,2);
  RobotState[myNumber].w:=strtofloatdef(mw,2);
